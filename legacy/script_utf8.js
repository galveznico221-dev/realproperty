/* ============================================================
   RPT SYSTEM ΓÇö TREASURER'S OFFICE
...
/* ============================================================
   1. MASTER DATA ΓÇö per Section VI-A-3 of the proposal
   ============================================================ */
const MASTER_DATA = {
  basicRate:       0.01,   // Provincial Basic Rate (1%)
  sefRate:         0.01,   // Special Education Fund rate
  discountAdvance: 0.20,   // 20% ΓÇö paid before start of tax year
  discountQ1:      0.10,   // 10% ΓÇö paid Jan to Mar
...
const PROPERTIES = [
  ...
  { pin:'002-0035', owner:'SANGGUNIANG BARANGAY NG POBLACION',
    ...
    remarks:'Barangay Hall ΓÇö tax-exempt under RA 7160' },
  ...
  { pin:'002-0042', owner:'PARISH OF ST. JOHN THE BAPTIST',
    ...
    remarks:'Church ΓÇö tax-exempt under RA 7160' },
...
let PAYMENTS = [
  { or:'2026-0001', date:'2026-01-15', pin:'002-0021', taxpayer:'DELA CRUZ, MARIA L.',
    ...
    encodedBy:'Treasurer IT Staff', remarks:'Paid in advance ΓÇö Q1 discount' },
  { or:'2026-0002', date:'2026-02-20', pin:'002-0028', taxpayer:'REYES, ANTONIO B.',
    ...
    encodedBy:'Treasurer IT Staff', remarks:'Paid within Q1 ΓÇö 10% discount' },
  { or:'2026-0003', date:'2026-06-10', pin:'002-0018', taxpayer:'RAMOS, JOSEFINA M.',
    ...
    encodedBy:'Treasurer IT Staff', remarks:'June payment ΓÇö 3 months penalty' }
];

let AUDIT_LOG = [
  { ts:'2026-01-15 09:12', user:'treasurer', action:'PAYMENT_RECORDED',
    ref:'OR 2026-0001', remarks:'Advance payment ΓÇö 20% discount applied' },
  { ts:'2026-02-20 14:05', user:'treasurer', action:'PAYMENT_RECORDED',
    ref:'OR 2026-0002', remarks:'Q1 payment ΓÇö 10% discount applied' },
  { ts:'2026-06-10 10:41', user:'treasurer', action:'PAYMENT_RECORDED',
    ref:'OR 2026-0003', remarks:'June payment ΓÇö 2% ├ù 3 months penalty' }
];
...
const peso = n => 'Γé▒' + Number(n || 0).toLocaleString('en-PH',
  { minimumFractionDigits: 2, maximumFractionDigits: 2 });
const peso0 = n => 'Γé▒' + Number(n || 0).toLocaleString('en-PH',
...
function computeTax(property, paymentDate, taxYear){
  if (property.taxExempt){
    return {
      ...
      penaltyMonths:0, penaltyRate:0, penalty:0, penaltyLabel:'ΓÇö',
  ...
  if (payYear < taxYear){
    ...
    discountLabel = '10% (paid JanΓÇôMar)';
  }
  ...
  const penaltyLabel = penaltyMonths
    ? `${penaltyMonths} month${penaltyMonths > 1 ? 's' : ''} ├ù 2%`
    : 'None';
...
function setView(name){
  ...
  document.title = `${title} ΓÇö RPT System | LGU Talisay`;
...
function renderDelinquent(){
  ...
  $('#delinquentTable').innerHTML = rows.length
    ...
    : `<tr><td colspan="7" class="empty">All taxable properties have recorded payments. ≡ƒÄë</td></tr>`;
}
...
function renderDetail(pin){
  ...
  $('#detailPanel').innerHTML = `
    ...
    <div class="detail">
      <h4>Computed Tax ΓÇö ${CURRENT_YEAR}</h4>
      ${c.exempt
...
function populateTxForm(){
  $('#txPin').innerHTML =
    '<option value="">ΓÇö Select a property record ΓÇö</option>' +
    PROPERTIES.map(p =>
      `<option value="${esc(p.pin)}">${esc(p.pin)} ΓÇö ${esc(p.owner)}</option>`
    ).join('');
...
function renderTxPreview(){
  ...
  box.innerHTML = `
    ...
    <div style="margin-top:6px">
      FMV <b>${peso(p.fairMarketValue)}</b> ├ù
  ...
  comp.innerHTML = `
    ...
    <div class="comp-row comp-row--discount">
      <span>Discount ΓÇö ${esc(c.discountLabel)}</span>
      <span>ΓêÆ ${peso(c.discount)}</span>
    </div>
    <div class="comp-row comp-row--penalty">
      <span>Penalty ΓÇö ${esc(c.penaltyLabel)}</span>
...
$('#btnGenerateSOA').addEventListener('click', () => {
  ...
  $('#btnRecordPayment').textContent = c.exempt
    ? 'Tax-Exempt ΓÇö No Payment Required'
...
function buildSOA({ property:p, computation:c, remarks }){
  ...
  const computeBlock = c.exempt ? '' : `
    <div class="soa__compute">
      ...
      <div class="comp-row comp-row--discount">
        <span>Discount ΓÇö ${esc(c.discountLabel)}</span><span>ΓêÆ ${peso(c.discount)}</span>
      </div>
      <div class="comp-row comp-row--penalty">
        <span>Penalty ΓÇö ${esc(c.penaltyLabel)}</span><span>+ ${peso(c.penalty)}</span>
      </div>
      <div class="comp-row comp-row--total">
        <span>TOTAL TAX DUE</span><span>${peso(c.total)}</span>
      </div>
  ...
  return `
    ...
    <div class="soa__sign">
      <div>
        <div class="line">Prepared by ΓÇö Treasurer's Office IT Staff</div>
      </div>
      <div>
        <div class="line">Approved by ΓÇö Municipal Treasurer</div>
      </div>
    </div>
  `;
}
...
$('#btnRecordPayment').addEventListener('click', () => {
  ...
  pushAudit('PAYMENT_RECORDED', `OR ${or}`,
    `${peso(c.total)} ΓÇö PIN ${p.pin} ΓÇö TY ${c.taxYear}`);
...
function buildReceipt(pay){
  ...
  return `
    <div class="receipt__check" aria-hidden="true">Γ£ô</div>
    ...
    <dl class="dl">
      ...
      <dt>Discount Applied</dt><dd>ΓêÆ ${peso(pay.discount)}</dd>
...
function renderCollections(){
  ...
  $('#collectionsTable').innerHTML = list.length
    ? list.map(p => `
        <tr>
          ...
          <td class="ta-r">ΓêÆ${peso(p.discount)}</td>
  ...
  $('#collectionsFoot').innerHTML = `
    <tr>
      ...
      <td class="ta-r">ΓêÆ${peso(totals.discount)}</td>
...
/* Tax due excluding penalty ("less penalty") ΓÇö basic + SEF ΓêÆ discount */
function taxNoPenalty(c){
  return c.exempt ? 0 : (c.basic + c.sef - c.discount);
}

/* Signed peso display ΓÇö keeps the ┬▒ sign in front of the Γé▒, e.g. "ΓêÆΓé▒480.00" */
function pesoSigned(n){
  if (n > 0) return '+' + peso(n);
  if (n < 0) return 'ΓêÆ' + peso(Math.abs(n));
  return peso(0);
}
...
$('#btnGenerateReport').addEventListener('click', () => {
  ...
  if (type === 'certified'){
    ...
    html = reportHeader('Certified List',
      ...
      ${signBlock("Prepared by ΓÇö Treasurer's Office IT Staff", 'Certified correct ΓÇö Municipal Treasurer')}`;
  }
  ...
  /* ---------- 2. CERTIFIED LIST (NO PENALTY) ---------- */
    ...
    html = reportHeader('Certified List (No Penalty)',
      ...
      <p style="margin-top:18px;line-height:1.7">
        THIS IS TO CERTIFY that the foregoing is a true and correct list of registered real
        properties and their corresponding tax due ΓÇö <b>exclusive of accrued penalty</b> ΓÇö as
  ...
  /* ---------- 3. LIST OF RPT (per-property tax register card) ---------- */
    ...
    function half(list, h){
      ...
      if (!set.length) return { amount: 0, discPen: 0, or: 'ΓÇö', datePaid: 'ΓÇö' };
      return {
    ...
    html = reportHeader('List of RPT', 'Real Property Tax Register Card') + `
      ...
      <p class="muted small" style="margin-top:10px">
        Note: quarters are grouped 1st/3rd and 2nd/4th per the legacy tax register card layout ΓÇö
        amounts reflect payments posted in the first half (JanΓÇôJun) and second half (JulΓÇôDec)
  ...
  /* ---------- 4. NOTICE OF DELINQUENCY (LESS PENALTY) ---------- */
    ...
    html = reportHeader('Notice of Delinquency (Less Penalty)',
      ...
      <p style="margin-bottom:18px;line-height:1.7">
        Pursuant to Sections 249ΓÇô256 of Republic Act No. 7160 (Local Government Code), the
          ...
          <tbody>
            ${rows.length ? rows.map(({p, c}) => `
              <tr>
                ...
                <td class="ta-r"><b>${peso(taxNoPenalty(c))}</b></td>
              </tr>`).join('') : '<tr><td colspan="5" class="empty">No delinquent accounts. ≡ƒÄë</td></tr>'}
          </tbody>
          <tfoot>
      ...
      ${signBlock("Prepared by ΓÇö Treasurer's Office IT Staff", 'Noted by ΓÇö Municipal Treasurer')}`;
  }
  ...
  /* ---------- 5. STATEMENT OF ACCOUNT (shortcut) ---------- */
    ...
    html = `
      <div class="empty">
        <div class="empty__ico" aria-hidden="true">Γé▒</div>
  ...
  /* ---------- 6. CERTIFICATION ---------- */
    ...
    html = `
      <div class="soa">
        ...
        ${signBlock("Prepared by ΓÇö Treasurer's Office IT Staff", 'HON. MUNICIPAL TREASURER<br><span class="muted small">Municipal Treasurer</span>')}
      </div>`;
  }
  ...
  /* ---------- 7. CERTIFICATION (QUARTERLY) ---------- */
    ...
    html = `
      <div class="soa">
        ${docLetterhead('CERTIFICATION ΓÇö QUARTERLY PAYMENT STATUS')}
        <div class="soa__meta">
          <div><b>Cert. No.</b><span class="mono">CERT-Q${Date.now().toString().slice(-8)}</span></div>
          <div><b>Date Issued</b><span>${new Date().toLocaleDateString('en-PH',{year:'numeric',month:'long',day:'numeric'})}</span></div>
          <div><b>PIN</b><span class="mono">${esc(p.pin)}</span></div>
          <div><b>Tax Year / Quarter</b><span>${year} ΓÇö ${qLabel} Quarter</span></div>
        </div>
        <p style="margin:18px 0;line-height:1.7">
